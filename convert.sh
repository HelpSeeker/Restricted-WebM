...
# Define help text
usage () {
echo -e "Usage: sh $0 [-h] [-t] [-a] [-p] [-m { variable | constant | low-variable }] [-s file_size_limit]"
echo -e "\t-h: Show help"
echo -e "\t-t: Enable trim mode. Lets you specify which part of the input video(s) to encode"
echo -e "\t-a: Enables audio encoding"
echo -e "\t-p: Enables VP8's two pass encoding. Only recommended for high bitrates (e.i. short webms)"
...
echo -e "\t-s file_size_limit: Specifies the file size limit in MB. Default value is 4."
echo -e "\t\t4chan limits:"
...
[[ -z $file_size ]] && file_size=4

# The main conversion loop
for input in *; do (
	
	# Find out length of the video for further calculations
	...
	if [[ "$two_pass" = true ]]; then
		...
		ffmpeg -y -ss $start_time -i "$input" -t $duration -c:v libvpx -slices 8 -threads 1 -metadata title="${input%.*}" -auto-alt-ref 1 -lag-in-frames 16 -bufsize $bufsize $video_settings -deadline best -cpu-used 0 $audio_settings -pass 2 "../${input%.*}.webm"
		rm ffmpeg2pass-0.log
	else
		ffmpeg -y -ss $start_time -i "$input" -t $duration -c:v libvpx -slices 8 -threads 1 -metadata title="${input%.*}" -lag-in-frames 16 -bufsize $bufsize $video_settings -deadline best -cpu-used 0 $audio_settings "../${input%.*}.webm"
	fi
